...
const GuideTab = ( { settings, onGoToTab } ) => {
	...
	// "enable" reflects the real, saved toggle instead of a separate manual
	// flag, so the checklist can't lie about whether the plugin is on.
	const isDone = ( id ) =>
		id === 'enable' ? !! ( settings && settings.enabled ) : !! checks[ id ];

	const doneCount = CHECKLIST.filter( ( item ) => isDone( item.id ) ).length;
	...
	return (
		<div className="cf-guide">
			<aside className="cf-guide__nav" aria-label={ __( 'Guide sections', 'consentaro' ) }>
				<p className="cf-guide__nav-title">{ __( 'On this page', 'consentaro' ) }</p>
				<ul>
					{ SECTIONS.map( ( s ) => (
						<li key={ s.id }>
							<button
			...
			<div className="cf-guide__content">
				<section id="cf-guide-intro" className="cf-guide__section">
					<div className="cf-guide__hero">
						<span className="cf-guide__badge">{ __( 'Start here', 'consentaro' ) }</span>
				...
				<section id="cf-guide-start" className="cf-guide__section">
					...
					<ul className="cf-guide__checklist">
						{ CHECKLIST.map( ( item ) => {
							const auto = item.id === 'enable';
							const done = isDone( item.id );
							return (
								<li key={ item.id }>
									<label
										className={
											auto ? 'cf-guide__checklist-auto' : undefined
										}
									>
										<input
											type="checkbox"
										...
										/>
										<span className={ done ? 'is-done' : undefined }>
											{ item.label }
											{ auto && (
												<em className="cf-guide__auto-tag">
													{ ' ' }
													{ __(
														'(synced with General)',
														'consentaro'
